	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() instanceof JButton) {
			...
			if (btn.equals(btnConnect)) {// connect button
				if (!txtServerHost.getText().isEmpty() && (txtServerHost.getText().trim().length() > 0) && !txtServerPort.getText().isEmpty() && (txtServerPort.getText().trim().length() > 0)) {// custom port
			 ...
			 * else if (ae.getSource() instanceof JMenuItem) { JMenuItem mni = (JMenuItem)
			 * ae.getSource(); if (mni.equals(mniLogin)) {// login function } else if
			 * (mni.equals(mniEditClient)) {// Search client to edit for (ObjectWrapper func
			 * : myControl.getActiveFunction()) if (func.getData() instanceof
			 * SearchCustomerFrm) { ((SearchCustomerFrm) func.getData()).setVisible(true);
			 * return; } SearchCustomerFrm scv = new SearchCustomerFrm(myControl);
			 * scv.setVisible(true); } }
			 */
	}
